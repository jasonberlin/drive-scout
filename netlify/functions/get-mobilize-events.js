exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('📡 Using API URL:', mobilizeUrl);
    ...
    if (data.data && Array.isArray(data.data)) {
      data.data.forEach((event, index) => {
        ...
        if (!hasDriveTag) {
          return; // Skip non-drive events
        }
        
        driveEventCount++;
        console.log(`🚗 Found Drive event #${driveEventCount}: ${event.title || 'Untitled'}`);
    ...
    console.log(`📊 Found ${driveEventCount} total drive events`);
    console.log(`📊 Processed ${processedEvents.length} valid drive events`);
    ...
    console.log(`🌴 California drive events: ${caEvents.length}`);
